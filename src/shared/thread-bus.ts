...
export const THREAD_TOOL_DESCRIPTIONS = {
  ...
  pluginCreate: 'Create the directory for a new BOSS plugin and write its plugin.json. Use it when the user asks for a capability BOSS does not have — a task board, a timer, a dashboard — that is worth keeping. It returns the directory to write into: put the MCP server and the view HTML there with your file tools, then call boss_plugin_reload. Two rules worth knowing before you write the server. A plugin\'s view reaches its data only by calling that plugin\'s own tools, so put every piece of state behind a tool. And BOSS adds a "project" field to every tool call ({projectId, projectPath}, "global" when no project is open) — store under project.projectId when the data belongs to one repository, ignore it when the plugin should be the same everywhere, and allow the field in your tool schemas rather than rejecting it with additionalProperties: false.',
