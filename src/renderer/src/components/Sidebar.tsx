...
export function Sidebar(): React.JSX.Element {
  ...
  // Review lists threads by what they need from the user rather than by where
  // they live. The supervision snapshot is the only place attention and results
  // exist, so this panel polls it — the sidebar's own session list has neither.
   ...
   *  a decision, then what is still going. A thread appears in exactly one
    ...
    <aside className="sidebar" style={{ width: sidebarWidth, flexBasis: sidebarWidth }}>
      ...
      <div className="sidebar-section review" hidden={tab !== 'review'}>
            ...
            <div className="list">
              {bucket.threads.map((entry) => (
                <div
                  ...
                  onClick={() => openThreadReview(entry.thread.threadId)}
        ...
        {reviewBuckets.length === 0 && (
          <div className="sidebar-empty">{supervision ? 'Nothing to review yet' : 'Loading…'}</div>
        )}
      </div>
