...
function SetupView({ identityName }: { identityName: string }): React.JSX.Element {
  ...
  return (
    <div className="team-empty-state">
      ...
      <p>Host a lightweight task board or connect to a teammate's BOSS Each developer claims work and runs it locally with their own backend, credentials, and worktrees.</p>
...
function TaskCard({ task, me, onOpen, onClaim, onStart, onMove }: {
  ...
  return (
    <article
      ...
      draggable
      onDragStart={(event) => event.dataTransfer.setData('application/x-boss-team-task', task.id)}
      onClick={onOpen}
    >
...
export function TeamBoardPage(): React.JSX.Element {
  ...
  return <div className="team-page">
    ...
    <section className="team-context-row">
      ...
      <div className="team-people-card"><div className="team-section-title"><span>People</span></div><div className="team-member-list">{board.members.map((member) => <div className={`team-member ${member.id === me ? 'self' : ''}`} key={member.id}><span>{member.name.slice(0, 1).toUpperCase()}</span><div><strong>{member.name}{member.id === me ? ' (you)' : ''}</strong><small>{member.deviceName ?? 'BOSS peer'} · {timeAgo(member.lastSeenAt)}</small></div></div>)}</div></div>
      {team.mode === 'host' ? <div className="team-share-card"><div className="team-section-title"><span>Invite · v{team.protocol.current}</span></div>{mobile?.running ? <><p>Share these over your tailnet. The token only grants access to this versioned board protocol.</p><button onClick={() => void copy(shareUrl, 'url')}><span>{shareUrl}</span><CopyIcon size={13} />{copied === 'url' ? 'Copied' : 'Copy'}</button><button onClick={() => void copy(accessToken, 'token')}><span className="team-token">{accessToken}</span><CopyIcon size={13} />{copied === 'token' ? 'Copied' : 'Copy token'}</button></> : <><p>Turn on Mobile & Tailscale access in Settings to invite another BOSS</p><button onClick={() => appStore.setState({ settingsOpen: true })}>Open Settings</button></>}</div> : null}
    </section>
